...
int dfs1(int node, int par)
{
    ...
    for (int next : adj[node])
    {
        if (next == par)
            continue;
        elems += dfs1(next, node);
    }
    return subtree_elems[node] = elems;
}
int find_centroid(int node, int par)
{
    // debug(node, par);
    int child_with_mx_elems = -1;
    int mx_child_elems = -1;
    for (int next : adj[node])
    {
        if (next == par)
            continue;
        if (mx_child_elems < subtree_elems[next])
            mx_child_elems = subtree_elems[next], child_with_mx_elems = next;
    }
    // debug(child_with_mx_elems, mx_child_elems);
    if (mx_child_elems <= N / 2)
        return node;
    else
        return find_centroid(child_with_mx_elems, node);
}
void solve()
{
    cin >> N;

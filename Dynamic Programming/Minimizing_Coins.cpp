...
array<int, NMAX> min_coins_possible, coins;
void solve()
{
    cin >> N >> sum;
    forab(i, 1, N + 1) { cin >> coins[i]; }
    fill(all(min_coins_possible), -1);
    min_coins_possible[0] = 0;
    forab(coin_idx, 1, N + 1)
    {
        const int &curr_coin = coins[coin_idx];
        forab(curr_sum, curr_coin, sum + 1)
        {
            if (min_coins_possible[curr_sum - curr_coin] == -1) continue;

            if (min_coins_possible[curr_sum] == -1)
                min_coins_possible[curr_sum] = min_coins_possible[curr_sum - curr_coin] + 1;
            else
                min_coins_possible[curr_sum] = min(min_coins_possible[curr_sum - curr_coin] + 1, min_coins_possible[curr_sum]);
        }
    }
    cout << min_coins_possible[sum] << endl;
}
int32_t main()
{

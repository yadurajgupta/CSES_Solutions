...
/* TEMPLATE END */
#pragma endregion
int N, x, y, Q, target_price;
const int NMAX = 1e6 + 5;
array<int, NMAX> prices;
array<int, NMAX> pages;
array<int, NMAX> max_pages;
array<int, NMAX> prev_max_pages;
void solve()
{
    cin >> N >> target_price;
    forab(i, 1, N + 1) { cin >> prices[i]; }
    forab(i, 1, N + 1) { cin >> pages[i]; }
    prev_max_pages.fill(-1);
    prev_max_pages[0] = 0;
    forab(book_idx, 1, N + 1)
    {
        forab(curr_price, 0, target_price + 1)
        {
            if (curr_price + prices[book_idx] <= target_price &&
                prev_max_pages[curr_price] != -1)
            {
                if (max_pages[curr_price + prices[book_idx]] == -1)
                    max_pages[curr_price + prices[book_idx]] = prev_max_pages[curr_price] + pages[book_idx];
                else
                    max_pages[curr_price + prices[book_idx]] = max(max_pages[curr_price + prices[book_idx]],
                                                                   prev_max_pages[curr_price] + pages[book_idx]);
            }
        }
        debug2(prices[book_idx], pages[book_idx]);
        debugab(max_pages, 1, target_price);
        forab(curr_price, 0, target_price + 1)
        {
            prev_max_pages[curr_price] = max_pages[curr_price];
        }
    }
    cout << *max_element(max_pages.begin(), max_pages.begin() + target_price + 1) << endl;
}
int32_t main()
{

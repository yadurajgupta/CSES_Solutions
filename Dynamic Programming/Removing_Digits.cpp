...
array<int, NMAX> minimum_number_of_steps;
int find_min_steps(const int num)
{
    if (minimum_number_of_steps[num] == -1)
    {
        if (num <= 9) return minimum_number_of_steps[num] = 1;
        int ans = LLONG_MAX;
        int num_mutable = num;
        while (num_mutable)
        {
            if (num_mutable % 10)
                ans = min(ans, 1 + find_min_steps(num - num_mutable % 10));
            num_mutable /= 10;
        }
        minimum_number_of_steps[num] = ans;
    }
    return minimum_number_of_steps[num];
}
void solve()
{
    cin >> N;
    minimum_number_of_steps.fill(-1);

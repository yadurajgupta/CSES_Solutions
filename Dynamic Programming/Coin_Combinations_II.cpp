...
void solve()
{
    ...
    {
        forab(curr_sum, coins[coin_idx], target_sum + 1)
        {
            if (!number_of_ways[curr_sum - coins[coin_idx]]) continue;
            number_of_ways[curr_sum] += number_of_ways[curr_sum - coins[coin_idx]];
            number_of_ways[curr_sum] %= MOD;
        }
    }

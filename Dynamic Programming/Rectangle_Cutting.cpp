...
array<array<int, NMAX>, NMAX> memoized_ans;
int find_mininmum_cuts(int a, int b)
{
    if (a > b)
        swap(a, b);
    if (memoized_ans[a][b] != -1) return memoized_ans[a][b];
    if (a == b) return 0;
    int ans = LLONG_MAX;
    forab(i, 1, a)
    {
        ans = min(ans, 1 + find_mininmum_cuts(i, b) + find_mininmum_cuts(a - i, b));
    }
    forab(j, 1, b)
    {
        ans = min(ans, 1 + find_mininmum_cuts(a, j) + find_mininmum_cuts(a, b - j));
    }
    return memoized_ans[a][b] = ans;
}
void solve()
{
    cin >> x >> y;
    for (auto &row : memoized_ans)
        row.fill(-1);

    cout << find_mininmum_cuts(x, y) << endl;
}
int32_t main()
{

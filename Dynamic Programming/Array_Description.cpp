...
const int MMAX = 101;
array<int, NMAX> arr;
array<array<int, MMAX>, NMAX> memo;
const int MOD = 1e9 + 7;
int modAdd(int a, int b) { return ((a % MOD + b % MOD) % MOD + MOD) % MOD; }
int modMult(int a, int b) { return (((a % MOD) * (b % MOD)) % MOD + MOD) % MOD; }
int recur(int arr_idx, int last_ele)
{
    const int &curr_val = arr[arr_idx];

    if (arr_idx == N + 1) return 1;
    if (last_ele != 0 && curr_val != 0 && abs(last_ele - curr_val) > 1) return 0;
    if (memo[arr_idx][last_ele] != -1) return memo[arr_idx][last_ele];
    ...
    if (curr_val == 0)
    {
        ...
        if (last_ele == 0)
        {
            forab(i, 1, M + 1) // nothing in current or prev val
            {
                ans = modAdd(recur(arr_idx + 1, i), ans);
            }
        }
        else
        {
            forab(i, -1, 2) // nothing in prev val
            {
                if (1 <= last_ele + i && last_ele + i <= M)
                {
                    ans = modAdd(recur(arr_idx + 1, last_ele + i), ans);
                }
            }
        }
        return memo[arr_idx][last_ele] = ans;
    }
    else // if(curr_val!=0)
    {
        return memo[arr_idx][last_ele] = recur(arr_idx + 1, curr_val);
    }
}
void solve()
{

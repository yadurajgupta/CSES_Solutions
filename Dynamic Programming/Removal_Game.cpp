...
array<array<array<pair<bool, int>, 2>, NMAX>, NMAX> memoized_max_first_person_score;
array<int, NMAX> score;
int make_move(int start, int end, bool first_player_turn)
{
    debug3(start, end, first_player_turn);
    if (start == end)
        return first_player_turn ? score[start] : 0;

    if (memoized_max_first_person_score[start][end][first_player_turn].first)
        return memoized_max_first_person_score[start][end][first_player_turn].second;

    if (first_player_turn)
        memoized_max_first_person_score[start][end][first_player_turn] =
            {true, max({score[start] + make_move(start + 1, end, !first_player_turn),
                        score[end] + make_move(start, end - 1, !first_player_turn)})};
    else
        memoized_max_first_person_score[start][end][first_player_turn] =
            {true, min({make_move(start + 1, end, !first_player_turn),
                        make_move(start, end - 1, !first_player_turn)})};
...
{
    cin >> N;
    forab(i, 1, N + 1) cin >> score[i];

    cout << make_move(1, N, true) << endl;
}
int32_t main()
{

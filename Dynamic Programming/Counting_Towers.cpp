...
int modAdd(const int &a, const int &b) { return ((a % MOD + b % MOD) % MOD + MOD) % MOD; }
int modMult(const int &a, const int &b) { return (((a % MOD) * (b % MOD)) % MOD + MOD) % MOD; }
...
void solve()
{
    ...
    //     y = ny;
    // }
    // cout << modAdd(x, y) << endl;

    Matrix<2, 2> base({{{2, 1}, {1, 4}}});
    Matrix<1, 2> ini_vals{{{1, 1}}};
    const auto &result = multiply_matrix(ini_vals, matrix_to_pow(base, N - 1));
    cout << modAdd(result(0, 0), result(0, 1)) << endl;
}
int32_t main()
{

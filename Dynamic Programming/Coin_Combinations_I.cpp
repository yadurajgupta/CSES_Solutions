...
array<int, NMAX> number_of_ways_to_make_sum;
int32_t MOD = 1e9 + 7;
...
void solve()
{
    ...
    sort(coins.begin() + 1, coins.begin() + N + 1);
    number_of_ways_to_make_sum[0] = 1;
    forab(curr_sum, 1, sum + 1)
    {
        forab(coin_idx, 1, N + 1)
        {
            if (curr_sum < coins[coin_idx]) break;
            number_of_ways_to_make_sum[curr_sum] += number_of_ways_to_make_sum[curr_sum - coins[coin_idx]];
            number_of_ways_to_make_sum[curr_sum] %= MOD;
        }
    }
    cout << number_of_ways_to_make_sum[sum] << endl;
}
int32_t main()
{

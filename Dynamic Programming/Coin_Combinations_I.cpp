...
/* TEMPLATE END */
#pragma endregion
int N, x, y, Q, target_sum;
const int NMAX = 1e6 + 5;
int coins[NMAX];
int number_of_ways[NMAX];
const int MOD = 1e9 + 7;
inline int modAdd(int a, int b) { return ((a % MOD + b % MOD) % MOD + MOD) % MOD; }
inline int modMult(int a, int b) { return (((a % MOD) * (b % MOD)) % MOD + MOD) % MOD; }
void solve()
{
    cin >> N >> target_sum;
    ...
    number_of_ways[0] = 1;
    forab(curr_sum, 1, target_sum + 1)
    {
        forab(coin_idx, 1, N + 1)
        {
            if (curr_sum < coins[coin_idx]) break;
            if (!number_of_ways[curr_sum - coins[coin_idx]]) continue;
            number_of_ways[curr_sum] = (number_of_ways[curr_sum] +
                                        number_of_ways[curr_sum - coins[coin_idx]]) %
                                       MOD;
        }
    }
    cout << number_of_ways[target_sum] << endl;
}
int32_t main()
{

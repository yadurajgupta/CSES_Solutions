...
struct SegmentTree
{
    ...
    void update_node_based_on_children()
    {
        if (is_leaf)
            return;
        this->val = max(lower->val, upper->val);
    }
    pair<bool, int> query_tree(const int target_max) const
    {
        if (val < target_max)
            return {false, INVALID_ANSWER};
        if (is_leaf)
            return {true, index_l};
        const auto &[lower_valid, lower_ans] = lower->query_tree(target_max);
        if (lower_valid)
            return {lower_valid, lower_ans};
        return upper->query_tree(target_max);
    }
};
...
void solve()
{
    ...
    forab(i, 1, N + 1) cin >> arr[i];
    {
        ...
        {
            cin >> number_of_guest;
            const auto &[is_valid, ans] = SegmentTree::query_segment_tree(root, number_of_guest);
            if (is_valid)
            {
                SegmentTree::update_segment_tree(root, ans, arr[ans] - number_of_guest);
                arr[ans] -= number_of_guest;
                cout << ans;
            }
            else
            {

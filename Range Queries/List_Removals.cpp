...
array<int, NMAX> arr;
array<int, NMAX> prefix_index_arr;
struct SegmentTree
{
    int val;
    bool is_valid;
    ...
    static pair<bool, int> query_segment_tree(shared_ptr<SegmentTree> &root, const int target_max)
    {
        return root->query_tree(0, target_max);
    }
    ...
    void update_node_based_on_children()
    {
        ...
        this->is_valid = lower->is_valid || upper->is_valid;
    }
    pair<bool, int> query_tree(int curr_prefix_sum, const int &target_sum) const
    {
        if (!is_valid || !(curr_prefix_sum <= target_sum && target_sum <= curr_prefix_sum + val))
            return {false, INVALID_ANSWER};

        if (is_leaf)
        {
            if (curr_prefix_sum == target_sum)
                return {true, index_l};
            return {false, INVALID_ANSWER};
        }
        const auto &[lower_valid, lower_ans] = lower->query_tree(curr_prefix_sum, target_sum);
        if (lower_valid)
            return {lower_valid, lower_ans};
        return upper->query_tree(lower->val + curr_prefix_sum, target_sum);
    }
};
...
void solve()
{
    ...
    fill(prefix_index_arr.begin(), prefix_index_arr.begin() + N + 1, 1);
    {
        shared_ptr<SegmentTree> root = SegmentTree::build_segment_tree(prefix_index_arr, 1, N);
        int apparent_index, actual_index;
        forab(i, 0, N)
        {
            cin >> apparent_index;
            actual_index = SegmentTree::query_segment_tree(root, apparent_index - 1).second;
            cout << arr[actual_index] << " ";
            SegmentTree::update_segment_tree(root, actual_index, 0);
        }
    }
}
int32_t main()

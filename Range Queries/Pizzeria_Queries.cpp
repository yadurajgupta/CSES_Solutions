...
array<int, NMAX> arr;
array<int, NMAX> upper;
array<int, NMAX> lower;
...
struct SegmentTree
{
    ...
    static pair<bool, int> query_segment_tree(shared_ptr<SegmentTree> &root, const int &target_l, const int &target_r)
    {
    ...
    void update_node_based_on_children()
    {
        if (is_leaf)
            return;
        this->val = min(lower->val, upper->val);
    }
    pair<bool, int> query_tree(const int &target_l, const int &target_r) const
    {
        ...
        else if (overlap(target_l, target_r) && !is_leaf)
        {
            ...
            return {true, min(lower_ans, upper_ans)};
        }
...
void solve()
{
    ...
    {
        cin >> arr[index];
        lower[index] = arr[index] + (N - index + 1);
        upper[index] = arr[index] + (index);
    }

    {
        int tp, price, index;
        shared_ptr<SegmentTree> lower_tree = SegmentTree::build_segment_tree(lower, 1, N);
        shared_ptr<SegmentTree> upper_tree = SegmentTree::build_segment_tree(upper, 1, N);
        forab(i, 0, Q)
        {
            cin >> tp;
            if (tp == 1)
            {
                cin >> index >> price;
                arr[index] = price;
                SegmentTree::update_segment_tree(lower_tree, index, arr[index] + (N - index + 1));
                SegmentTree::update_segment_tree(upper_tree, index, arr[index] + (index));
            }
            else if (tp == 2)
            {
                cin >> index;
                auto const &[lower_valid, lower_ans] = SegmentTree::query_segment_tree(lower_tree, 1, index);
                auto const &[upper_valid, upper_ans] = SegmentTree::query_segment_tree(upper_tree, index, N);
                cout << min({lower_ans - (N - index + 1), upper_ans - (index)}) << endl;
            }
        }
    }
}

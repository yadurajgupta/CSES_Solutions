...
struct SegmentTree
{
    ...
    bool is_upto_date = true;

    bool has_set_type_update_left = false;
    int value_for_set_update = 0;

    bool has_increment_type_update_left = false;
    int value_for_increment_update = 0;
    ...
    static void update_segment_tree(SegmentTree *&root, const int &update_node_index_s, const int &update_node_index_e, const int &update_node_new_val, const bool &update_type_is_set)
    {
        if (!root)
            return;
        root->update_node(update_node_index_s, update_node_index_e, update_node_new_val, update_type_is_set);
        return;
    }
    ...
    void reset_all_update_values()
    {
        is_upto_date = true;
        has_set_type_update_left = false;
        value_for_set_update = 0;
        has_increment_type_update_left = false;
        value_for_increment_update = 0;
    }
    void force_update_and_lazy_prop_to_child()
    {
        if (is_leaf)
        {
            if (has_set_type_update_left)
            {
                val = value_for_set_update * get_interval_length();
            }
            if (has_increment_type_update_left)
            {
                val += value_for_increment_update * get_interval_length();
            }
            reset_all_update_values();
            return;
        }
        if (has_set_type_update_left)
        {
            ...
            lower->reset_all_update_values();
            lower->is_upto_date = false;
            lower->has_set_type_update_left = true;
            lower->value_for_set_update = value_for_set_update;

            upper->reset_all_update_values();
            upper->is_upto_date = false;
            upper->has_set_type_update_left = true;
            upper->value_for_set_update = value_for_set_update;
        }
        if (has_increment_type_update_left)
        {
            val += value_for_increment_update * get_interval_length();
            ...
            lower->is_upto_date = false;
            lower->has_increment_type_update_left = true;
            lower->value_for_increment_update += value_for_increment_update;

            upper->is_upto_date = false;
            upper->has_increment_type_update_left = true;
            upper->value_for_increment_update += value_for_increment_update;
        }
        reset_all_update_values();
    }
    void update_node(const int &update_node_index_s, const int &update_node_index_e, const int &update_node_new_val, const bool &update_type_is_set)
    {
        if (!enclosed_in(update_node_index_s, update_node_index_e) && !overlap(update_node_index_s, update_node_index_e)) return;
        if (is_leaf)
        {
            if (!is_upto_date) force_update_and_lazy_prop_to_child();

            if (update_type_is_set)
                val = update_node_new_val * get_interval_length();
            else //update type is increment
                val += update_node_new_val * get_interval_length();
        }

        else if (enclosed_in(update_node_index_s, update_node_index_e))
        {
            // set lazy prop on this
            if (update_type_is_set)
            {
                if (!is_upto_date) reset_all_update_values();

                is_upto_date = false;
                has_set_type_update_left = true;
                value_for_set_update = update_node_new_val;
            }
            else //update type is increment
            {
                is_upto_date = false;
                has_increment_type_update_left = true;
                value_for_increment_update += update_node_new_val;
            }
        }
        else if (overlap(update_node_index_s, update_node_index_e))
        {
            if (!is_upto_date) this->force_update_and_lazy_prop_to_child();
            lower->update_node(update_node_index_s, update_node_index_e, update_node_new_val, update_type_is_set);
            upper->update_node(update_node_index_s, update_node_index_e, update_node_new_val, update_type_is_set);
    ...
    void update_node_based_on_children()
    {
        if (is_leaf)
            return;
        lower->force_update_and_lazy_prop_to_child();
        upper->force_update_and_lazy_prop_to_child();
        this->val = lower->val + upper->val;
    }
    pair<bool, int> query_tree(const int &target_l, const int &target_r)
    {
        if (!enclosed_in(target_l, target_r) && !overlap(target_l, target_r)) return {false, INVALID_ANSWER};
        if (!is_upto_date) force_update_and_lazy_prop_to_child();
...
void solve()
{
...

    {
        ...
        int query_type, s_idx, e_idx, set_value, increment_value;
        forab(i, 0, Q)
        {
            cin >> query_type >> s_idx >> e_idx;
            if (query_type == 1)
            {
                cin >> increment_value;
                SegmentTree::update_segment_tree(root, s_idx, e_idx, increment_value, /* update_type_is_set = */ false);
            }
            else if (query_type == 2)
            {
                cin >> set_value;
                SegmentTree::update_segment_tree(root, s_idx, e_idx, set_value, /* update_type_is_set = */ true);
            }
            else if (query_type == 3)
            {
                cout << SegmentTree::query_segment_tree(root, s_idx, e_idx).second << endl;
            }
        }
        delete root;
    }

...
void solve()
{
    ...
    forab(i, 1, N + 1) prefix[i] = arr[i] - arr[i - 1];

    {
        int tp, a, b, u, k;
        shared_ptr<SegmentTree> root = SegmentTree::build_segment_tree(prefix, 1, N);
        forab(i, 0, Q)
        {
            cin >> tp;
            if (tp == 1)
            {
                cin >> a >> b >> u;
                SegmentTree::update_segment_tree(root, a, prefix[a] + u);
                prefix[a] += u;
                if (b + 1 <= N)
                {
                    SegmentTree::update_segment_tree(root, b + 1, prefix[b + 1] - u);
                    prefix[b + 1] -= u;
                }
            }
            else if (tp == 2)
            {
                cin >> k;
                const auto &[validity, ans] = SegmentTree::query_segment_tree(root, 1, k);
                if (validity)
                    cout << ans << endl;
            }
        }

...
array<multiset<int>, NMAX> k_shorest;
void solve()
{
    cin >> N >> M >> K;
    for (int i = 0; i < M; i++)
    ...
    while (!st.empty())
    {
        ...
        {
            if (k_shorest[next].size() == K && *(k_shorest[next].rbegin()) > dist + wei)
                k_shorest[next].erase(prev(k_shorest[next].end()));
            if (k_shorest[next].size() < K)
            {
                k_shorest[next].insert(dist + wei);
                st.insert({dist + wei, next});
            }
        }
    }
    print(k_shorest[N]);

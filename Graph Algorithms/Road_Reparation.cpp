...
void dfs(int node, int curr_parent)
{
    if (parent[node] == curr_parent)
        return;

    parent[node] = curr_parent;
    for (int next : adj[node])
        dfs(next, curr_parent);
}
void solve()
{
    cin >> N >> M;
    ...
    int ans = 0;
    for (auto &[wei, src, des] : edges)
    {
        if (parent[src] != parent[des])
        {
            dfs(src, min(parent[src], parent[des]));
            dfs(des, min(parent[src], parent[des]));

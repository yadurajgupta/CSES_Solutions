...
int MOD = 1e9 + 7;
int modAdd(int a, int b, int MOD) { return (a % MOD + b % MOD) % MOD; }
int modMult(int a, int b, int MOD) { return ((a % MOD) * (b % MOD)) % MOD; }
int modPow(int a, int b, int MOD) //a^b
{
    if (a == 0 && b == 0)
        return 1;
    if (a == 0 || a == 1)
        ...
        return modMult(a, modPow(modMult(a, a, MOD), (b - 1) / 2, MOD), MOD);
    else
        return modPow(modMult(a, a, MOD), b / 2, MOD);
}
void solve()
{
    cin >> a >> b >> c;

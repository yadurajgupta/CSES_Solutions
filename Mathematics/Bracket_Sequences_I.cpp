...
int recur(int open_left, int close_left, int N)
{
    auto key = mkp(open_left, close_left);
    if (mp.count(key)) return mp[key];
    // debug2(open_left, close_left);

    if (open_left == 0)
        return mp[key] = 1;
    else if (close_left > 0)
        return mp[key] = modAdd(recur(open_left - 1, close_left + 1, N), recur(open_left, close_left - 1, N));
    else
        return mp[key] = recur(open_left - 1, close_left + 1, N);
}
void solve()
{
    cin >> N;
    ...
    N /= 2;
    int num = fac[2 * N];
    num = modMult(num, fac_inv[N]);
    num = modMult(num, fac_inv[N]);
    num = modMult(num, modInverse(N + 1));
    cout << num << endl;
    // (1/(n+1)) * (2NCN);
    // cout << recur(N, 0, N) << endl;
}
int32_t main()
{
    precalc();
